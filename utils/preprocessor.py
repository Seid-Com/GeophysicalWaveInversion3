...
class SeismicPreprocessor:
    ...
    def _apply_bandpass_filter(self, seismic_data: np.ndarray, lowcut: float = 5.0, highcut: float = 30.0, fs: float = 100.0) -> np.ndarray:
    ...
    def competition_preprocessing(self, seismic_data: np.ndarray, velocity_data: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
        Competition-focused preprocessing pipeline
        Based on winning approach analysis
        """
        # Aggressive noise filtering for seismic data
        processed_seismic = self.preprocess_seismic_data(seismic_data, apply_filters=True)
        
        # Z-score normalization with clipping
        processed_seismic = self._robust_normalize(processed_seismic)
        
        # Minimal velocity preprocessing (keep it simple)
        processed_velocity = np.clip(velocity_data, 1500, 6000)
        processed_velocity = self._normalize_velocity_range(processed_velocity)
        
        return processed_seismic, processed_velocity
    ...
    def _robust_normalize(self, data: np.ndarray) -> np.ndarray:
        ...
        if mad > 0:
            normalized = (data - median) / (1.4826 * mad)  # Scale factor for normal distribution
